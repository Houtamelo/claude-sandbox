...
## Install
    ...
    make install              # builds + installs to ~/.local/bin/claude-sandbox
                              #   and seeds ~/.config/claude-sandbox/{Dockerfile,config.toml}
    claude-sandbox cfg        # interactive machine-setup wizard:
                              #   - host UID
                              #   - base image (default debian:trixie-slim)
                              #   - optional long-lived Claude OAuth token
    claude-sandbox rebuild    # builds the base image (claude-sandbox:0.1)

Every subcommand except `cfg`, `init`, `--help`, `--version` requires that
`cfg` has been run at least once.
